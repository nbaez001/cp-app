...
@Component({
  selector: 'app-solicitar-evaluacion',
  templateUrl: './solicitar-evaluacion.component.html',
  styleUrls: ['./solicitar-evaluacion.component.scss']
})

export class SolicitarEvaluacionComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<SolicitarEvaluacionComponent>,
  ...
  solicitar($event) {
    ...
    dialogMessage.afterClosed().subscribe((confirm: boolean) => {
      ...
      if (confirm) {
        ...
        this.asignarEquipoElaboradorJefeProyecto();
  ...
  public openDialogMensaje2(
    ...
    dialogRef.afterClosed().subscribe((ok: number) => {
     ...
     let asignarProfesionalesDialog: Object = {
       valorDevolver: "PROFESIONAL_ELABORADOR",
  ...
  asignarEquipoElaboradorJefeProyecto(): void{
    debugger;
    ...
    this.proyectoService.asignarEquipoElaboradorEncargado(this.profesionalRequest)
    .subscribe(
  ...
  public obtenerArquitecto(idproyectoMov: string) : void{

    this.profesionalArquitecto = [];
    this.personalService.obtenerArquitectos(idproyectoMov).subscribe(
  ...
  public obtenerIngenieroCivil(idproyectoMov: string) : void{

    this.profesionalCivil = [];
    this.personalService.obtenerCiviles(idproyectoMov).subscribe(
      (wsResponsePersonal : WsResponsePersonal)=> {
        if(wsResponsePersonal.codResultado == 1){
  ...
  public obtenerIngenieroElectrico(idproyectoMov: string) : void{

    this.profesionalElectrico = [];
    this.personalService.obtenerElectricos(idproyectoMov).subscribe(
      (wsResponsePersonal : WsResponsePersonal)=> {
        if(wsResponsePersonal.codResultado == 1){
  ...
  public obtenerIngenieroSanitario(idproyectoMov: string) : void{

    this.profesionalSanitario= [];
    this.personalService.obtenerSanitarios(idproyectoMov).subscribe(
      (wsResponsePersonal : WsResponsePersonal)=> {
        if(wsResponsePersonal.codResultado == 1){
  ...
  public obtenerOtros(idproyectoMov: string) : void{
    
    this.profesionalOtros= [];
    this.personalService.obtenerOtros(idproyectoMov).subscribe(
      (wsResponsePersonal : WsResponsePersonal)=> {
        if(wsResponsePersonal.codResultado == 1){

...
@Component({
  ...
  styleUrls: ['./registro-particular.component.scss']
})
export class RegistroParticularComponent implements OnInit {
  ...
  validarRegistro($event){
    ...
    if(this.proyectoForm.get('frmCondicionParticular').value == '' || (this.proyectoForm.get('frmCondicionParticular').value == null)){
    ...
    .subscribe(
      (wsResponseTdr : WsResponseTdr)=> {
        ...
        if(wsResponseTdr.codResultado == 1){ 
          ...
          this.openDialogMensaje2(MENSAJES.TDR.TITLE_CONDICION_PARTICULAR, null, this.mensaje ,true, false, "OK");
        }else{
